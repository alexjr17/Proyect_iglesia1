                ...
                @slot('indices')
                    ...
                    {{-- @if ($indices)
                        @foreach ($indices as $indice)
                            <th {{ $indice == 'Nombre' || $indice == '' ? 'data-priority=1' : ''}}" scope="col"
                                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {{ $indice }}
                            </th>
                        @endforeach
                    ...
                    <tr>
                        <td class="px-2 py-2 pl-4">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    @isset($miembro->image)
                                        <img class="h-10 w-10 rounded-full" src="{{ Storage::url($miembro->image->url) }}"
                                            alt="">
                                    @else
                        ...
                        <td class="py-2 px-2 flex justify-center">
                            @can('admin.miembros.edit')
                                <a class="btn-edit mr-2" href="{{ route('admin.miembros.edit', $miembro) }}">Editar</a>
                            @endcan
